...
def rpn_loss_z(scores, deltas, inds, pos_inds, rpn_labels, rpn_targets,deltas_z,targets_z):
    ...
    deltas1_z       = tf.reshape(deltas_z,[-1,2])
    targets_z       =tf.reshape(targets_z,[-1,2])
    rpn_deltas_z    = tf.gather(deltas1_z, pos_inds)  # remove ignore label
    rpn_smooth_l1_z = modified_smooth_l1(rpn_deltas_z, targets_z, sigma=3.0)
    rpn_reg_loss_z  = tf.reduce_mean(tf.reduce_sum(rpn_smooth_l1_z, axis=1))

    return rpn_cls_loss, rpn_reg_loss, rpn_reg_loss_z